...
import {
  PreviewDataResponse,
  GenerateRegexTasksRequest,
  GenerateRegexTasksResponse,
...
export const generateRegexTasks = async (
  data: GenerateRegexTasksRequest
): Promise<GenerateRegexTasksResponse> => {
  try {
    const response = await api.post<GenerateRegexTasksResponse>('/generate-tasks/', data);
    return response.data;
  } catch (error) {
    throw handleApiError(error);
  }
